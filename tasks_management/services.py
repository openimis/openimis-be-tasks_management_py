...
def on_task_complete_service_handler(service_type: Type[BaseService]):
    ...
    UpdateCheckerLogicServiceMixin, DeleteCheckerLogicServiceMixin. I will automatically detect available
    ...
    def func(**kwargs):
        try:
            ...
            if result and result['success'] \
                    and result['data']['task']['business_event'].startswith(f"{service_type.__name__}.") \
                    and result['data']['task']['status'] == Task.Status.COMPLETED:
                operation = result['data']['task']['business_event'].split(".")[1]
                if operation in operations:
                    user = User.objects.get(id=result['data']['user']['id'])
                    data = result['data']['task']['data']
